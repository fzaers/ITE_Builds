...
function _IsMobileBrowser() {
 ...
 var isMobile = /\b(BlackBerry|webOS|iPhone|IEMobile)\b/i.test(userAgent) || /\b(Android|Windows Phone|iPad|iPod)\b/i.test(userAgent) || userAgent.includes("Mac") && "ontouchend" in document;
 return isMobile;
}
...
function _PhotonVoice_WebAudioAudioOut_Start(handle, sampleRate, channels, bufferSamples, spatialBlend) {
 ...
 const addProc = function(audioContext) {
  const playProc = new AudioWorkletNode(audioContext, "photon-voice-play-processor", {
   processorOptions: {
    channels: channels,
 ...
 audioContext.audioWorklet.addModule(Module.PhotonVoice_WebAudioAudioOut_Global.PlayWorkerURL).then(function() {
...
function _PhotonVoice_WebAudioMicIn_Start(handle, deviceId, createCallback, dataCallback) {
 ...
 if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
  ...
  audioContext.audioWorklet.addModule(url).then(function(x) {
   navigator.mediaDevices.getUserMedia({
    audio: deviceIdStr == "" ? true : {
     deviceId: {
      exact: deviceIdStr
     }
    }
    ...
    worklet.port.onmessage = function(e) {
     ...
     dataHeap.set(b);
     (function(a1, a2, a3) {
      dynCall_viii.apply(null, [ dataCallback, a1, a2, a3 ]);
     })(handle, ptr, b.length);
     _free(ptr);
    };
    audioContext.resume();
    ...
    (function(a1, a2, a3, a4) {
     dynCall_viiii.apply(null, [ createCallback, a1, a2, a3, a4 ]);
    })(handle, 0, audioContext.sampleRate, 1);
   }).catch(function(err) {
...
function _PhotonVoice_WebRTC_EnumerateDevices(requestId, kind, resultCallback) {
 ...
 if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia && navigator.mediaDevices.enumerateDevices) {
  navigator.mediaDevices.getUserMedia({
   audio: kindStr.startsWith("audio"),
   video: kindStr.startsWith("video")
  }).then(function(stream) {
   navigator.mediaDevices.enumerateDevices().then(devices => {
    ...
    (function(a1, a2, a3, a4) {
     dynCall_viiii.apply(null, [ resultCallback, a1, a2, a3, a4 ]);
    })(requestId, 0, ptr, res.length);
    for (let i = 0; i < res.length; i++) {
     _free(ptr[i]);
    }
    _free(ptr);
...
function _Photon_JS_TimerWorker_Start(callback, interval) {
 ...
 worker.onmessage = (() => {
  (function(a1) {
   dynCall_vi.apply(null, [ callback, a1 ]);
  })(handle);
 });
...
function _SocketCreate(url, protocols, openCallback, recvCallback, errorCallback, closeCallback) {
 ...
 socket.socket.onopen = function() {
  (function(a1) {
   dynCall_vi.apply(null, [ openCallback, a1 ]);
  })(instance);
 };
 socket.socket.onmessage = function(e) {
  if (e.data instanceof ArrayBuffer) {
   ...
   (function(a1, a2, a3) {
    dynCall_viii.apply(null, [ recvCallback, a1, a2, a3 ]);
   })(instance, ptr, b.byteLength);
   _free(ptr);
  }
 };
 socket.socket.onerror = function(e) {
  (function(a1, a2) {
   dynCall_vii.apply(null, [ errorCallback, a1, a2 ]);
  })(instance, e.code);
 };
 socket.socket.onclose = function(e) {
  if (e.code != 1e3) {
   (function(a1, a2) {
    dynCall_vii.apply(null, [ closeCallback, a1, a2 ]);
   })(instance, e.code);
  }
 };
 return instance;
...
if (!Object.getOwnPropertyDescriptor(Module, "webcamBufferToTextureTable")) Module["webcamBufferToTextureTable"] = function() {
 abort("'webcamBufferToTextureTable' was not exported. add it to EXPORTED_RUNTIME_METHODS (see the FAQ)");
};

if (!Object.getOwnPropertyDescriptor(Module, "webcamLatestTextureId")) Module["webcamLatestTextureId"] = function() {
 abort("'webcamLatestTextureId' was not exported. add it to EXPORTED_RUNTIME_METHODS (see the FAQ)");
};

if (!Object.getOwnPropertyDescriptor(Module, "colorMaskValue")) Module["colorMaskValue"] = function() {
 abort("'colorMaskValue' was not exported. add it to EXPORTED_RUNTIME_METHODS (see the FAQ)");
};
